...
def call_args():
    ...
    parser.add_argument('--descr_path',
                        type=str, default='/workspace/local/MegaDepth_DoG/dense/512_hard',
                        help='')
...
class MegaDepthset():
    ...
    def __getitem__(self, idx):
        ...
        keypoints_x = torch.from_numpy(np.load(self.desc_path / 'val' /f'{img_name}.{self.descriptor}')['keypoints_x'])
        keypoints_y = torch.from_numpy(np.load(self.desc_path / 'val' /f'{img_name}.{self.descriptor}')['keypoints_y'])
        descriptors = torch.from_numpy(np.load(self.desc_path / 'val' /f'{img_name}.{self.descriptor}')['descriptors'])
